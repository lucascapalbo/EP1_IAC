...
void InicializaAFD(tAFD *aut, int n, int s) {
    ...
    aut->inacessivel = (int*) calloc(n, sizeof(int));
    for (i=0; i<n; i++) aut->inacessivel[i] = 1;
    
    aut->inutil = (int*) calloc(n, sizeof(int));
    for (i=0; i<n; i++) aut->inutil[i] = 1;
...
/*
 ...
 Lay-out:
 A 1a linha deve conter o n�mero de estados, numero de simbolos do alfabeto e indice do estado inicial.
